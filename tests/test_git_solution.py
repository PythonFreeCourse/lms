...
class TestSendSolutionFromGit:
    ...
    def setup_method(self, _: str) -> None:
        self.temp_folder = tempfile.mkdtemp()

    def teardown_method(self, _: str) -> None:
        if self.temp_folder and os.path.exists(self.temp_folder):
            shutil.rmtree(self.temp_folder)

    @staticmethod
    def _get_formatted_git_url(
        exercise: models.Exercise, rel_path: str,
    ) -> str:
    ...
    def _send_git_request(
        ...
        password: str = conftest.FAKE_PASSWORD,
    ):
        ...
        with mock.patch("lms.lmsweb.views.REPOSITORY_FOLDER", self.temp_folder):
            return getattr(client, method_name)(
                url, query_string=query_string, headers=headers, data=data,
            )

    def test_not_authorized_access(
        self, exercise: models.Exercise, student_user: models.User
    ):
