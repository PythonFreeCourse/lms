...
def confirm_email(user_id: int, token: str):
    ...
    try:
        SERIALIZER.loads(
            token,
            salt=retrieve_salt(user),
            max_age=CONFIRMATION_TIME,
        )
...
@webapp.route(
    "/recover-password/<int:user_id>/<token>",
    methods=["GET", "POST"],
)
@limiter.limit(f"{LIMITS_PER_MINUTE}/minute;{LIMITS_PER_HOUR}/hour")
...
def upload_page(course_id: int):
    ...
    if request.content_length > MAX_UPLOAD_SIZE:
        return fail(
            413,
            f"File is too big. {MAX_UPLOAD_SIZE // 1000000}MB allowed.",
        )
...
babel.init_app(webapp, locale_selector=get_locale)
for m in ALL_MODELS:
